...
%addpath k-Wave/
% Update end
addpath '/Users/danielhawkey/Documents/MATLAB /MATLAB_PROJECT/FUS_data/OneWay_RM001/'
...
INPUT_DATA_DIR = {['/Users/danielhawkey/Documents/MATLAB /MATLAB_PROJECT/FUS_data/OneWay_RM001/' ...
    '']}; %'Y:/ScanData/FUSIntrs/';
...
for n = 1%: length(INPUT_DATA_DIR)
  [~, mess] = fileattrib([INPUT_DATA_DIR{1}, '/*.mat']);
    ...
    for m = 1:length(mess)
        ...
        % --- DIAGNOSTIC: print key values before processing ---
        fprintf('\n--- File %d: %s ---\n', m, filename)
        fprintf('ScanAxes: %s\n', ScanData.ScanAxes)
        fprintf('dt_meas = %.3e\n', ScanData.samplePeriod(1))
        fprintf('t0 range: %.3e to %.3e\n', min(ScanData.startTime), max(ScanData.startTime))
        fprintf('Voltage size: %d x %d\n', size(ScanData.Voltage,1), size(ScanData.Voltage,2))
        ...
        % After loading ScanData, always extract Z and compute t_arrival
        % regardless of scan axis — move this OUTSIDE the if/elseif branches
        Z_all = abs(ScanData.posZ * 1e-3);
        z_mean = mean(Z_all(:));
        c_temp = waterSoundSpeed(mean(ScanData.Temperature));
        t_arrival = z_mean / c_temp;   % approximate arrival time at this plane

        % Also always define dt and t0 before the branching
        data.dt_meas = ScanData.samplePeriod(1);
        data.t0      = ScanData.startTime;
        pts_cycle    = 1 / (TRANSDUCER_FREQ * data.dt_meas);
        data_cut     = round(n_cycles * pts_cycle);

        % Find peak voltage position
        [~, i_p] = maxND(ScanData.Voltage);

        % Now compute i_start safely
        ...
        fprintf('File %d (%s): t_arrival=%.2e, ind_arrival=%.1f, i_start=%d, data_cut=%d\n', ...
            m, ScanData.ScanAxes, t_arrival, ind_arrival, i_start, data_cut)

        if ~isfinite(i_start) || i_start < 1
            warning('i_start is invalid (%g) for file %d — skipping', i_start, m)
            continue
        end
        if i_start + data_cut > size(ScanData.Voltage, 2)
            warning('Window extends beyond data length for file %d — clamping', m)
            data_cut = size(ScanData.Voltage, 2) - i_start - 1;
        end
        ...
        if strcmp(ScanData.ScanAxes, 'X')
            ...
            plot(data.X*1e3, data.p_magX/1e3, 'DisplayName', shortname)
        ...
        elseif strcmp(ScanData.ScanAxes, 'Y')
            ...
            hold all
            [~, shortname, ~] = fileparts(filename);
            plot(data.Y*1e3, data.p_magY/1e3,'DisplayName', shortname)
            legend('show', 'Interpreter', 'none')
            % calculate the -3dB and -6dB focal width 
            % (0.7 and 0.5 in pressure, 0.5 and 0.25 in intensity)
...
% plot the frequency of the input waveform
figure
hold on;
